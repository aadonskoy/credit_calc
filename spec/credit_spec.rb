...
RSpec.describe Credit do
  ...
  describe 'data missing' do
    ...
    let(:no_percent)   { { 'amount' => 100000, 'term' => 10, 'cred_meth' => 1 } }
    let(:no_term)      { { 'amount' => 100000, 'percent' => 10, 'cred_meth' => 1 } }
    let(:no_cred_meth) { { 'amount' => 100000, 'percent' => 10, 'term' => 6 } }
  ...
  describe 'validation' do
    ...
    let(:wrong_percent)   { { 'amount' => 100000, 'percent' =>  0, 'term' => 10, 'cred_meth' => 1  } }
    let(:wrong_term)      { { 'amount' => 100000, 'percent' => 10, 'term' => 0,  'cred_meth' => 1  } }
    let(:wrong_cred_meth) { { 'amount' => 100000, 'percent' => 10, 'term' => 6,  'cred_meth' => 43 } }
    let(:correct_data)    { { 'amount' => 100000, 'percent' => 10, 'term' => 6,  'cred_meth' => 1 } }

    context 'valid data' do
      it { expect(Credit.new(correct_data).valid?).to be(true) }
      it { expect(Credit.new(correct_data).validate.length).to be(0) }
    end
    ...
    context 'not valid data' do
      ...
      context 'error message' do
        subject do
          Credit.new({ 'amount' => 0, 'percent' => 0, 'term' => 0, 'cred_meth' => -100 }).validate.length
        end
  ...
  describe 'good data' do
    let(:data_default) { { 'amount' => 100000, 'percent' => 10, 'term' => 6,  'cred_meth' => 1 } }
    let(:data_annuitent) { { 'amount' => 100000, 'percent' => 10, 'term' => 6,  'cred_meth' => 2 } }

    context 'attributes' do
      it { expect(Credit.new(data_default).cred_meth).to include('Стандартный')}
      it { expect(Credit.new(data_annuitent).cred_meth).to include('Аннуитентный')}
